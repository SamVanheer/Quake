...
int WIPX_Read (int handle, byte *buf, int len, struct qsockaddr *addr)
{
	...
	if (ret == -1)
	{
		...
		if (ierrno == WSAEWOULDBLOCK || ierrno == WSAECONNREFUSED)
			return 0;
...
int WIPX_Write (int handle, byte *buf, int len, struct qsockaddr *addr)
{
	...
	*(int *)(&packetBuffer[0]) = sequence[handle];
	...
	ret = sendto (socket, reinterpret_cast<char*>( packetBuffer ), len, 0, (struct sockaddr *)addr, sizeof(struct qsockaddr));
	if (ret == -1)
