...
extern void D_PolysetSetEdgeTable (void);
extern void D_RasterizeAliasPolySmooth (void);
...
split:
// split this edge
	newPoly[0] = (lp1[0] + lp2[0]) >> 1;
	newPoly[1] = (lp1[1] + lp2[1]) >> 1;
	newPoly[2] = (lp1[2] + lp2[2]) >> 1;
	newPoly[3] = (lp1[3] + lp2[3]) >> 1;
	newPoly[5] = (lp1[5] + lp2[5]) >> 1;
...
// draw the point if splitting a leading edge
	...
	z = newPoly[5]>>16;
	zbuf = zspantable[newPoly[1]] + newPoly[0];
	if (z >= *zbuf)
	{
		int		pix;
		
		*zbuf = z;
		pix = d_pcolormap[skintable[newPoly[3]>>16][newPoly[2]>>16]];
		d_viewbuffer[d_scantable[newPoly[1]] + newPoly[0]] = pix;
	}

nodraw:
// recursively continue
	D_PolysetRecursiveTriangle (lp3, lp1, newPoly);
	D_PolysetRecursiveTriangle (lp3, newPoly, lp2);
}
