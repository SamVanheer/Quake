...
// build the combined string to parse from
	...
	text = reinterpret_cast<char*>( Z_Malloc (s+1) );
	text[0] = 0;
	for (i=1 ; i<com_argc ; i++)
	{
		if (!com_argv[i])
...
// pull out the commands
	build = reinterpret_cast<char*>( Z_Malloc (s+1) );
	build[0] = 0;
...
char *CopyString (const char *in)
{
	...
	out = reinterpret_cast<char*>( Z_Malloc (strlen(in)+1) );
	strcpy (out, in);
	return out;
}
...
// skip whitespace up to a /n
		...
		text = COM_Parse (const_cast<char*>(text));
		if (!text)
			return;
